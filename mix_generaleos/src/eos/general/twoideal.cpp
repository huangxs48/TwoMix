...
Real EquationOfState::GetEffectiveGamma(int k, int j, int i) {
  ...
  //Real gamma_effective = gamma_flow1 * pmy_block_->pscalars->r(0,k,j,i) + gamma_flow2 * pmy_block_->pscalars->r(1,k,j,i);
  Real gamma_effective = gamma_flow1 * pmy_block_->pscalars->r(0,k,j,i) + gamma_flow2 * (1.0 -pmy_block_->pscalars->r(0,k,j,i));
  //Real gamma_effective = gamma_flow1 * (1.0 - pmy_block_->pscalars->r(1,k,j,i))  + gamma_flow2 * pmy_block_->pscalars->r(1,k,j,i);
