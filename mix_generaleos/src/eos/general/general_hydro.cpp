...
#pragma omp simd
      for (int i=il; i<=iu; ++i) {
        ...
        w_p = PresFromRhoEg(u_d, u_e - ke, k, j, i);
      }
    }
  }
...
#pragma novector
      for (int i=il; i<=iu; ++i) {
        ...
        u_m3 = w_vz*w_d;
        // cellwise conversion
        u_e = EgasFromRhoP(u_d, w_p, k, j, i) + 0.5*w_d*(SQR(w_vx) + SQR(w_vy) + SQR(w_vz));
      }
    }
  }
...
Real EquationOfState::SoundSpeed(const Real prim[NHYDRO], int k, int j, int i) {
  return std::sqrt(AsqFromRhoP(prim[IDN], prim[IPR], k, j, i));
}
