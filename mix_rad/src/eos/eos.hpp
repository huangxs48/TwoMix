...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this)
  Real SoundSpeed(const Real prim[(NHYDRO)], int k, int j ,int i);
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this)
  ...
  Real PresFromRhoEg(Real rho, Real egas, int k, int j, int i);
  Real EgasFromRhoP(Real rho, Real pres, int k, int j, int i);
  Real AsqFromRhoP(Real rho, Real pres, int k, int j, int i);
...
#if GENERAL_EOS
  Real GetGamma();
  Real GetEffectiveGamma(int k, int j, int i);
