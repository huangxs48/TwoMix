...
#pragma distribute_point
  for (int i=il; i<=iu; ++i) {
    ...
    Real al, ar, el, er;
    Real cl = pmy_block->peos->SoundSpeed(wli, k, j, i);
    Real cr = pmy_block->peos->SoundSpeed(wri, k, j, i);
    if (GENERAL_EOS) {
      el = pmy_block->peos->EgasFromRhoP(wli[IDN], wli[IPR], k, j, i) +
           0.5*wli[IDN]*(SQR(wli[IVX]) + SQR(wli[IVY]) + SQR(wli[IVZ]));
      er = pmy_block->peos->EgasFromRhoP(wri[IDN], wri[IPR], k, j, i) +
    ...
    Real ql, qr;
    if (GENERAL_EOS) {
      Real gl = pmy_block->peos->AsqFromRhoP(rhol, pmid, k, j, i) * rhol / pmid;
      Real gr = pmy_block->peos->AsqFromRhoP(rhor, pmid, k, j, i) * rhor / pmid;
      ql = (pmid <= wli[IPR]) ? 1.0 :
